...
function renderStaff() {
    ...
    // Use dynamic staff config based on screen width and fragment length
    const config = getStaffConfig();
    const { lines, lineSpacing, topLine, leftMargin, noteSpacing } = config;
    const hs = lineSpacing / 2; // half-space
    const divisions = state.currentSongData?.divisions || 120;
    
    // ========================================================================
    // PROPORTIONAL SPACING CALCULATION
    // ========================================================================
    
    // Build layout items (notes + gaps for rests)
    const layoutItems = [];
    ...
    state.notePositions = positions;
    
    // Calculate SVG dimensions based on actual width used
    ...
    // ========================================================================
    // DRAW TREBLE CLEF & TIME SIGNATURE
    // ========================================================================
    
    const clefText = createSVGElement('text', {
        x: leftMargin - 38,
        y: topLine + 3 * lineSpacing, // G clef wraps around the G line (2nd from bottom)
        'font-size': lineSpacing * 3.5,
        'font-family': 'Bravura',
        fill: 'var(--fg)'
    });
    clefText.textContent = SMUFL.gClef;
    svg.appendChild(clefText);
    
    const timeSig = state.currentSongData?.timeSignature || { beats: 4, beatType: 4 };
    const timeSigDigits = [SMUFL.timeSig0, SMUFL.timeSig1, SMUFL.timeSig2, SMUFL.timeSig3,
        SMUFL.timeSig4, SMUFL.timeSig5, SMUFL.timeSig6, SMUFL.timeSig7, SMUFL.timeSig8, SMUFL.timeSig9];
    
    // Top number (beats)
    const beatsText = createSVGElement('text', {
        x: leftMargin - 5,
        y: topLine + lineSpacing * 1.5,
        'font-size': lineSpacing * 2.5,
        'font-family': 'Bravura',
        fill: 'var(--fg)',
        'text-anchor': 'middle'
    });
    beatsText.textContent = timeSigDigits[timeSig.beats] || timeSig.beats.toString();
    svg.appendChild(beatsText);
    
    // Bottom number (beat type)
    const beatTypeText = createSVGElement('text', {
        x: leftMargin - 5,
        y: topLine + lineSpacing * 3.5,
        'font-size': lineSpacing * 2.5,
        'font-family': 'Bravura',
        fill: 'var(--fg)',
        'text-anchor': 'middle'
    });
    beatTypeText.textContent = timeSigDigits[timeSig.beatType] || timeSig.beatType.toString();
    svg.appendChild(beatTypeText);
    ...
    if (keySig.fifths !== 0) {
        ...
        const symbol = isSharp ? SMUFL.accidentalSharp : SMUFL.accidentalFlat;
        
        // Standard staff positions for key signature accidentals (treble clef)
        // Position 0 = top line (F5), each +1 = one step down (half lineSpacing)
        // Sharps order: F C G D A E B — staff positions from top line
        const sharpPositions = [0, 3, -1, 2, 5, 1, 4]; // F5, C5, G5, D5, A4, E5, B4
        // Flats order: B E A D G C F — staff positions from top line
        const flatPositions = [4, 1, 5, 2, 6, 3, 7];   // B4, E5, A4, D5, G4, C5, F4
        
        const positions = isSharp ? sharpPositions : flatPositions;
        
        for (let i = 0; i < Math.min(count, 7); i++) {
            const staffPos = positions[i];
            const y = topLine + staffPos * (lineSpacing / 2);
            const x = leftMargin + 12 + i * 10;
            
            const keyAccidental = createSVGElement('text', {
                x: x,
                y: y + 16 * 0.1, // baseline adjustment for SMuFL
                'font-size': '16',
                'font-family': 'Bravura',
                fill: 'var(--fg)',
                'text-anchor': 'middle'
            });
            keyAccidental.textContent = symbol;
            svg.appendChild(keyAccidental);
        }
    }
    ...
    fragment.forEach((note, index) => {
        ...
        if (cumulativeDivisions >= divisionsPerMeasure && index < fragment.length - 1) {
            ...
            svg.appendChild(createSVGElement('line', {
                ...
                y1: topLine,
                x2: barlineX,
                y2: topLine + (lines - 1) * lineSpacing,
                stroke: 'var(--fg)',
                'stroke-width': 2
            }));
    ...
    // ========================================================================
    // DOUBLE BARLINE AT FRAGMENT END
    // ========================================================================
    ...
    // ========================================================================
    // PRE-COMPUTE NOTE DATA (using proportional positions)
    // ========================================================================
    
    const noteData = fragment.map((note, index) => {
        const x = positions[index];
        const midi = state.userPitches[index];
        const staffPos = midiToStaffPosition(midi);
        const y = topLine + 2 * lineSpacing - staffPos * hs;
        const stemUp = staffPos <= 0;
        return { x, y, midi, staffPos, stemUp, note, index };
    });
    
    // Identify beam groups: consecutive eighth/sixteenth notes, split at beat boundaries
    // In standard notation, beams group notes within the same beat (quarter note)
    const beatDiv = state.currentSongData ? state.currentSongData.divisions : 120;
    const beamGroups = [];
    let currentGroup = [];
    
    function getBeat(note) {
        // Which beat does this note start on? (0-based, each beat = divisions ticks)
        if (note.startDiv != null) return Math.floor(note.startDiv / beatDiv);
        return -1;
    }
    
    for (let i = 0; i < fragment.length; i++) {
        const type = fragment[i].type;
        if (type === 'eighth' || type === '16th') {
            // Break beam at beat boundaries
            if (currentGroup.length > 0) {
                const prevBeat = getBeat(fragment[currentGroup[currentGroup.length - 1]]);
                const curBeat = getBeat(fragment[i]);
                if (curBeat !== prevBeat && prevBeat >= 0 && curBeat >= 0) {
    ...
    // Set of indices that are beamed (no individual flags)
    const beamedIndices = new Set();
    beamGroups.forEach(g => g.forEach(i => beamedIndices.add(i)));
    
    // Draw each note
    noteData.forEach((nd, index) => {
        const { x, y, midi, staffPos, stemUp, note } = nd;
        
        // Ledger lines
        drawLedgerLines(svg, x, staffPos, lineSpacing, topLine);
        
        // Playback cursor handled by updatePlayhead() — no re-render needed
        
        // Notehead class
        const isPlaybackNote = state.playbackActive && index === state.playbackCursorIndex;
        const isCorrect = midi === note.midi;
        let noteheadClass = state.submitted 
            ? (isCorrect ? 'notehead correct' : 'notehead incorrect revealing')
            : isPlaybackNote ? 'notehead selected'
            : (index === state.selectedNoteIndex ? 'notehead selected' : 'notehead');
        
        // Draw notehead (tilted ellipse like real notation)
        drawNotehead(svg, x, y, note.type, noteheadClass);
        
        // Draw stem
        if (note.type !== 'whole') {
            drawStem(svg, x, y, stemUp);
        }
        
        // Draw flags only for unbeamed eighth/sixteenth notes
        if (!beamedIndices.has(index)) {
            if (note.type === 'eighth') drawFlag(svg, x, y, stemUp, 1);
            else if (note.type === '16th') drawFlag(svg, x, y, stemUp, 2);
        }
        ...
        if (needsAccidental(midi)) {
            ...
            const accEl = createSVGElement('text', {
                ...
                'font-size': fontSize,
                'font-family': 'Bravura',
                fill: noteheadClass.includes('correct') ? 'var(--correct)' :
                      noteheadClass.includes('incorrect') ? 'var(--incorrect)' :
                      noteheadClass.includes('selected') ? 'var(--selected)' : 'var(--fg)',
                'text-anchor': 'middle'
            });
        ...
        const isDotted = Math.abs(duration - quarterNote * 1.5) < 10 || 
        ...
        // Selected note indicator (arrow below) - PRIORITY 2.8: Smart tooltip positioning
        if (!state.submitted && index === state.selectedNoteIndex) {
            // Determine tooltip position based on note height
            const isHighNote = staffPos > 2;
            const isLowNote = staffPos < -4;
            
            let arrowY, tooltipY;
            if (isHighNote) {
                // Show below for high notes
                arrowY = topLine + (lines - 1) * lineSpacing + 15;
                tooltipY = arrowY + 20;
            } else if (isLowNote) {
                // Show above for very low notes
                arrowY = topLine - 15;
                tooltipY = arrowY - 8;
            } else {
                // Default below
            ...
            // Arrow indicator
            const arrowDir = (isHighNote || !isLowNote) ? 1 : -1;
            svg.appendChild(createSVGElement('polygon', {
                points: arrowDir > 0 
                    ? `${x},${arrowY} ${x - 6},${arrowY + 8} ${x + 6},${arrowY + 8}`
                    : `${x},${arrowY} ${x - 6},${arrowY - 8} ${x + 6},${arrowY - 8}`,
                fill: 'var(--selected)',
                class: 'note-arrow'
            }));
            ...
            // Note name tooltip
            const noteName = midiToNoteName(midi, true);
            const tooltipEl = createSVGElement('text', {
                x: x,
                y: tooltipY,
                'font-size': '10',
            ...
            tooltipEl.textContent = noteName;
    ...
    beamGroups.forEach(group => {
        drawBeamGroup(svg, group, noteData, fragment);
    });
    ...
    // Draw ties between consecutive notes of the same pitch
    for (let i = 0; i < fragment.length - 1; i++) {
        if (fragment[i].tieStart) {
            const x1 = positions[i];
            const x2 = positions[i + 1];
            const y1 = noteData[i].y;
            const y2 = noteData[i + 1].y;
            const stemUp = noteData[i].stemUp;
            
            // Tie curves away from the stems (below if stems up, above if stems down)
            const curveDir = stemUp ? 1 : -1;
            const midX = (x1 + x2) / 2;
            const midY = (y1 + y2) / 2 + curveDir * 12;
            
            // Draw tie as a quadratic bezier curve
            const d = `M ${x1 + 8} ${y1 + curveDir * 6} Q ${midX} ${midY} ${x2 - 8} ${y2 + curveDir * 6}`;
